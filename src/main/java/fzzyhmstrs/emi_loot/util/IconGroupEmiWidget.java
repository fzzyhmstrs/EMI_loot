...
public class IconGroupEmiWidget extends Widget {
    ...
    public IconGroupEmiWidget(int x, int y, ClientMobLootTable.ClientMobBuiltPool pool){
        this.x = x;
        this.y = y;
        ...
        this.icons = list;
        this.iconsWidth = 12 + (((icons.size() - 1)/2) * 11) + 2;
        this.width = iconsWidth + pool.stackMap().size() * 20;
        this.bounds = new Bounds(x,y, width,23);
        List<SlotWidget> list2 = new LinkedList<>();
        int itemXOffset = iconsWidth;
        for(Map.Entry<Float, EmiIngredient> entry: pool.stackMap().float2ObjectEntrySet()){
            SlotWidget widget = new SlotWidget(entry.getValue(),itemXOffset,y + 3).appendTooltip(LText.translatable("emi_loot.percent_chance",entry.getKey()));
            itemXOffset +=20;
            list2.add(widget);
        }
        items = list2;
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        DrawableHelper.fill(matrices,x,y,x,width,0x555555);
        for (IconEmiWidget icon: icons){
